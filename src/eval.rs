use crate::{expression::{ExpressionType, Expression, self}, builtin, context::Context};
...
pub fn eval(ctx: &Context, ast: Expression) -> Result<Expression> {
    Ok(match ast.as_ref() {
        ...
        ExpressionType::Block(exprs) => {
            ...
            for expr in exprs {
                block_value = eval(ctx,expr.to_owned())?;
            }
            block_value
        }
        ...
        ExpressionType::If(cond, then, r#else) => {
            debug!("eval if");
            let cond = eval(ctx,cond.to_owned())?;
            match cond.as_ref() {
                ExpressionType::Boolean(b) =>
                    if *b { eval(ctx,then.to_owned())? } 
                    else  { eval(ctx,r#else.to_owned())? }
                _ => ast.to_error()?,
            }
        }
            ...
            match eval(ctx,lambda.to_owned())?.as_ref() {
                ...
                ExpressionType::Closure(closure_ctx, arg_names, body) => {
                    ...
                    for (name,value) in arg_names.iter().zip(arg_values) {
                        function_ctx.add_binding(name.to_owned(), eval(ctx,value.to_owned())?);
                    }
                    ...
                    if arg_names.len() > arg_values.len() {
                        ...
                        let uncurried = eval(&function_ctx,body.to_owned())?;
                        eval(&function_ctx,ExpressionType::FunctionCall(uncurried,arg_values[arg_names.len()..].to_vec()).into())?
                    } else {
                        eval(&function_ctx,body.to_owned())?
                    }
                },
                ExpressionType::FunctionDynamic(arg_names, body) => {
                    ...
                    eval(ctx, ExpressionType::FunctionCall(closure, arg_values.to_owned()).into())?
                },
                _ => ast.to_error()?
            }
        }
        ...
        ExpressionType::BinOpCall(op, left, right) => {
            ...
            let right = eval(ctx,right.to_owned())?;
            match op {
                ExpressionType::InfixOp(fname) => {
                    if fname.starts_with('$') {
                        let fvar = ExpressionType::Variable(fname.strip_prefix('$').unwrap().to_owned()).into();
                        eval(ctx,ExpressionType::FunctionCall(fvar, vec![left,right]).into())?
                    } else {
                        Err(anyhow!("infix op bad prefix: {fname}"))?
                    }
                },
                ExpressionType::PipeOp => eval(ctx,ExpressionType::FunctionCall(right, vec![left]).into())?,
        ...
        ExpressionType::Array(vals) => {
            debug!("eval array");
            expression::array(
                vals.borrow().iter().map(|v|eval(ctx,v.to_owned())).collect::<Result<Vec<_>>>()?
            )
        }
        _ => ast.to_error()?,
    })
