...
// impl Context {
    pub fn eval(ctx: &mut Context, ast: &Expression) -> Expression {
        match ast {
            ...
            Expression::FunctionCall(lambda, arg_values) => {
                match eval(ctx,lambda) {
                    Expression::Builtin(name) => {
                        let eval_args : Vec<_> = arg_values.iter().map(|e|eval(ctx,e)).collect();
    ...
    pub fn builtin(ctx: &mut Context, name: &str, args: &[Expression]) -> Expression {
        match (name, args) {
